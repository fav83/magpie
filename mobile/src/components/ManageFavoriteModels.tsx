...
export function ManageFavoriteModels({ onBack, apiKey }: ManageFavoriteModelsProps): React.JSX.Element {
  ...
  const filtered = models?.filter((m) => {
    ...
    const matchesFree = !freeOnly || m.pricingDisplay === 'Free';
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="px-4 pt-4 pb-2 flex gap-2">
        <input
          ...
          className="flex-1 min-w-0 px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
        <button
          type="button"
          ...
          <>
            ...
            {favorites.map((m) => (
              <ModelRow
                ...
                model={m}
                isFavorite
                onToggle={() => void toggleFavorite(m.id)}
              />
            ))}
  ...
  return (
    <div className="flex items-center justify-between py-2.5 px-1">
      <div className="min-w-0 mr-3">
        ...
        <div className="text-xs text-gray-400">{model.pricingDisplay}</div>
      </div>
      <button
        type="button"
        onClick={onToggle}
