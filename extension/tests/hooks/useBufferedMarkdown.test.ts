...
describe('useBufferedMarkdown', () => {
  ...
  describe('flush', () => {
    it('should flush remaining buffer content', () => {
      ...
      // Append chunk with complete line and partial content in one act
      act(() => {
        result.current.appendChunk('complete line\npartial content');
        // Call flush in the same act to ensure we use the same hook instance
    ...
    it('should flush buffered content separately from displayed content', () => {
      ...
      // Verify displayed content only has complete line
      expect(result.current.displayContent).toBe('complete line\n');
      // Verify full content has everything
      expect(result.current.getFullContent()).toBe('complete line\npartial content');

      // Now flush separately - the buffer should still have 'partial content'
      act(() => {
        result.current.flush();
      });
  ...
  describe('reset', () => {
    ...
    it('should clear the buffer', () => {
      ...
      // After reset, flushing should add nothing
      act(() => {
        result.current.flush();
      });
